...
def add_random_site(site, start, cookie):
    """
    :param site:
    :param start:
    :param cookie:
        ...
        if response.status_code == 200:
            ...
            try:
                if int(result['status']) == -2:
                    print('添加数量超过上限，请调整后重新提交。')
                    return False
                elif int(result['status']) == -1:
                    print('添加数量超过上限，同一主站下可批量添加1000个子站，请调整后重新提交。')
                    return False
            except KeyError:
                pass
            for line in result['errList']:
                if int(line['status']) == 4:
                    print('添加网站--', line['url'], '--重复')
                elif int(line['status']) == 2:
                    print('添加网站--', line['url'], '--DNS解析失败，网站不存在')
                elif int(line['status']) == 0:
                    print('添加网站--', line['url'], '--成功')
                else:
                    print('添加网站--', line['url'], '--异常')
            return True
        else:
            return False
